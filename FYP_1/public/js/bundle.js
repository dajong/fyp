        ...
        $export(
          ...
          "Array",
          {
            ...
            from: function from(
              ...
              let length; var result; var step; var iterator;
              if (mapping)
...
let toNumber = function (argument) {
          ...
          if (typeof it == "string" && it.length > 2) {
            ...
            let third; var radix; var maxCode;
...
let global = require('./_global');
        ...
        let {MessageChannel} = global;
        let {Dispatch} = global;
        let counter = 0;
        let queue = {};
let ONREADYSTATECHANGE = 'onreadystatechange';
        let defer; var channel; var port;
let run = function () {
          let id = +this;
          // eslint-disable-next-line no-prototype-builtins
...
let empty = function () { /* empty */ };
let Internal; var newGenericPromiseCapability; var OwnPromiseCapability; var Wrapper;
...
let notify = function (promise, isReject) {
          ...
          microtask(function() {
            ...
            var run = function(reaction) {
              ...
              let {resolve} = reaction;
              let {reject} = reaction;
              let {domain} = reaction;
              let result; var then; var exited;
              try {
                if (handler) {
                  if (!ok) {
        ...
        var onUnhandled = function(promise) {
          task.call(global, function() {
            ...
            let result; var handler; var console;
            if (unhandled) {
...
let isObject = require('./_is-object');
        ...
        function set(target, propertyKey, V /* , receiver */) {
          ...
          let existingDescriptor; var proto;
          if (!ownDesc) {
            if (isObject((proto = getPrototypeOf(target)))) {
...
let SUPPORTS_Y = !fails(function () { RegExp(MAX_UINT32, 'y'); });
      ...
      let separatorCopy = new RegExp(separator.source, `${flags  }g`);
              let match; var lastIndex; var lastLength;
...
let toLength = require('./_to-length');
let isObject = require('./_is-object');
let {ArrayBuffer} = require('./_global');
let speciesConstructor = require('./_species-constructor');
...
let ARRAY_BUFFER = 'ArrayBuffer';
  ...
  let addGetter = function (it, key, internal) {
          ...
          let $from = function from(source /* , mapfn, thisArg */) {
            ...
            let mapping = mapfn !== undefined;
            let iterFn = getIterFn(O);
            let i; var length; var values; var result; var step; var iterator;
    ...
    let getter = function (that, index) {
            ...
            if (FORCED) {
              TypedArray = wrapper(function(that, data, $offset, $length) {
                ...
                let index = 0;
                let offset = 0;
                let buffer; var byteLength; var length; var klass;
                if (!isObject(data)) {
                  length = toIndex(data);
